...
class GroundingDinoClip:
    ...
    def test(self, text_prompt="subject", box_threshold=0.35, text_threshold=0.25, clip_threshold=0.9, random_seed=0, image=None, imageUrl=None):
        ...
        try:
            ...
            probs = self.step2_clip(cut_img_paths, clip_prompts)
            print("clip的结果",probs)

            # 获取每个一维数组最大值的索引
            max_indices = np.argmax(probs, axis=1)
            clip_results = [clip_prompts[idx] for idx in max_indices]
            print("clip的结果",clip_results)
            for i, idx in enumerate(max_indices):
                if probs[i][idx] > clip_threshold and clip_results[i] == phrases[i]:
    ...
    def step2_clip(self, img_paths, text_prompt):
        ...
        with torch.no_grad():
            ...
            logits_per_image, logits_per_text = self.model_clip(image, text)
